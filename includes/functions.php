...
require_once("db.php");
require_once '../package/fpdf/fpdf.php';
...
if (isset($_POST['accion'])) {
    switch ($_POST['accion']) {
        ...
        case 'existeEspecialidad':
            ...
            break;
        case 'muestra_ticket':
            muestraTicket($_POST['id_paciente']);
            break;
        case 'genera_ticket':
            generaTicket($_POST['id_paciente']);
            break;
    }
}
...
function muestraTicket($id_paciente)
{
    include "db.php";

    $consulta = "SELECT id, nombre, antecedentes, enfermedades, estado FROM pacientes WHERE id = $id_paciente";
    $resultado = mysqli_query($conexion, $consulta);
    ...
    if (mysqli_num_rows($resultado) > 0) {
        ...
        // Aquí puedes retornar o procesar los datos del paciente según tus necesidades.
...
function generaTicket($id_paciente)
{
    include "db.php";
    ...
    if (mysqli_num_rows($resultado) > 0) {
        ...
        $pdf = new FPDF();
        $pdf->AddPage();
        $pdf->Image('../img/logo.png', 10, 10, 25, 25, 'PNG'); 

        $pdf->SetFont('Arial', 'B', 14);
        $pdf->Cell(0, 30, 'Historial del Paciente', 0, 1, 'C'); 

        $pdf->SetFont('Arial', 'B', 10);
        $pdf->Cell(0, 10, 'SanaCitas', 0, 1, 'C');
        $pdf->Ln(2); 

        $pdf->SetFont('Arial', 'B', 12);
        $pdf->Cell(50, 10, 'Nombre y apellidos: ', 0);
        $pdf->SetFont('Arial', '', 12);
        $pdf->Cell(0, 10, $paciente['nombre'], 0, 1);
 
        $pdf->Ln(10); 

       
        $pdf->SetDrawColor(0, 0, 0);
        $pdf->Line(50, $pdf->GetY(), 50, $pdf->GetY() + 205); // Ajusta la altura según sea necesario

  
        $pdf->SetFont('Arial', 'B', 12);
        $pdf->Cell(50, 10, 'Antecedentes:', 0);
        $pdf->Ln(5);
        $pdf->SetFont('Arial', '', 12);
        
        $antecedentes = explode(',', $paciente['antecedentes']);
        foreach ($antecedentes as $antecedente) {
            $texto = '- ' . trim($antecedente);
            $pdf->Cell(50, 10, '', 0);
        ...
        $pdf->SetFont('Arial', 'B', 12);
        $pdf->Cell(50, 10, 'Enfermedades:', 0);
        $pdf->Ln(5);
        $pdf->SetFont('Arial', '', 12);
        
        $enfermedades = explode(',', $paciente['enfermedades']);
        foreach ($enfermedades as $enfermedad) {
            $texto = '- ' . trim($enfermedad);
            $pdf->Cell(50, 10, '', 0); // Espacio para la línea
            $pdf->MultiCell(0, 8, $texto);
        }
